RSpec.describe DryOpenApi::Operation do
  ...
  describe '.load' do
    ...
    let(:hash) do
      {
        ...
        'callbacks' => { 'Acallback' => callback_hash },
        'deprecated' => false,
    ...
    before do
      ...
      allow(DryOpenApi::Responses).to receive(:load).with(responses_hash).and_return(responses)
      ...
      allow(DryOpenApi::SecurityRequirement).to receive(:load).with(security_requirement_hash).and_return(security_requirement)
      allow(DryOpenApi::Server).to receive(:load).with(server_hash).and_return(server)
    end
