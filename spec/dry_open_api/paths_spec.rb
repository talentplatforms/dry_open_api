RSpec.describe DryOpenApi::Paths do
  ...
  describe '.load' do
    ...
    let(:hash) { { '/books' => double(:path_item_hash) } }
    let(:path_item) { double(:path_item) }

    before do
      allow(DryOpenApi::PathItem).to receive(:load).and_return(path_item)
    end

    it 'creates an instance from hash' do
      is_expected.to eq described_class.new('/books': path_item)
    end
  end
end
