RSpec.describe DryOpenApi::PathItem do
  ...
  describe '.load' do
    ...
    before do
      ...
      allow(DryOpenApi::Server).to receive(:load).and_return(server)
      allow(DryOpenApi::Parameter).to receive(:load).and_return(parameter)
      allow(DryOpenApi::Operation).to receive(:load).with(get_operation_hash).and_return(get_operation)
      allow(DryOpenApi::Operation).to receive(:load).with(post_operation_hash).and_return(post_operation)
    end
