RSpec.describe DryOpenApi::RequestBody do
  ...
  describe '.load' do
    ...
    let(:hash) do
      {
        'description' => 'desc',
        'content' => { 'application/json' => json_object_hash }
      }
    end
    ...
    before do
      allow(DryOpenApi::MediaType).to receive(:load).with(json_object_hash).and_return(json_object)
    end

    it 'creates an instance from hash' do
      is_expected.to eq described_class.new(
        description: 'desc',
        content: { 'application/json' => json_object }
      )
    end
  end
end
